...
| `part_2/data/curated/curated_applications.csv` | Stores the curated application dataset exported by the transformation pipeline. |
...
| `part_3/api/routers/export_router.py` | Defines CSV export endpoints for applications and yearly statistics. |
...
| `part_3/api/routers/refresh_router.py` | Defines the protected endpoint for refreshing the curated database table. |
...
| `part_3/api/schemas/response_schema.py` | Defines Pydantic response models for applications, refreshes, and statistics. |
| `part_3/api/services/application_service.py` | Queries curated application records and checks application existence. |
...
| `part_3/api/services/refresh_service.py` | Rebuilds the curated dataset and reloads it into PostgreSQL. |
...
| `part_3/sql/create_tables.sql` | Defines the PostgreSQL schema, tables, and indexes for curated data. |
...
| `src/myh_pipeline/config.py` | Stores pipeline configuration for source files, selected years, and mappings. |
| `src/myh_pipeline/enrich.py` | Adds derived analytical fields such as approval flags and sector categories. |
| `src/myh_pipeline/harmonize.py` | Maps yearly Excel dataframes into a shared curated schema. |
| `src/myh_pipeline/load.py` | Loads configured MYH Excel sheets and adds source traceability columns. |
| `src/myh_pipeline/pipeline.py` | Orchestrates loading, cleaning, harmonization, enrichment, and validation. |
| `src/myh_pipeline/validate.py` | Builds validation summaries for the curated dataset. |
